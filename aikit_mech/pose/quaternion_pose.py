...
from aikit_mech.pose import matrix_pose as aikit_mat_pose
...
def axis_angle_pose_to_quaternion_pose(axis_angle_pose):
    ...
    # BS x 7
    return aikit.concat(
        [translation, aikit_quat.axis_angle_to_quaternion(axis_angle_pose[..., 3:])],
        axis=-1,
    )
...
def mat_pose_to_quaternion_pose(matrix):
    ...
    # BS x 7
    return aikit.concat(
        [matrix[..., 0:3, -1], aikit_quat.rot_mat_to_quaternion(matrix[..., 0:3, 0:3])],
        axis=-1,
    )
...
def euler_pose_to_quaternion_pose(euler_pose, convention="zyx", batch_shape=None):
    ...
    return mat_pose_to_quaternion_pose(
        aikit_mat_pose.euler_pose_to_mat_pose(euler_pose, convention, batch_shape)
    )
...
def increment_quaternion_pose_with_velocity(quat_pose, quat_vel, delta_t):
    ...
    quaternion_transform = aikit_quat.scale_quaternion_rotation_angle(
        quaternion_vel, delta_t
    )
    new_quaternion = aikit_quat.hamilton_product(current_quaternion, quaternion_transform)
