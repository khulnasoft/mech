...
def euler_pose_to_axis_angle_pose(
    ...
    """
    aa = aikit_aa.euler_to_axis_angle(
        euler_pose[..., 3:], convention, batch_shape, device=None
    )
    return aikit.concat([euler_pose[..., :3], aa], axis=-1)
...
def mat_pose_to_rot_vec_pose(matrix):
    ...
    # BS x 4
    quaternion = aikit_quat.rot_mat_to_quaternion(matrix[..., 0:3])

    # BS x 3
    rot_vector = aikit_aa.quaternion_to_rotation_vector(quaternion)

    # BS x 6
    return aikit.concat([translation, rot_vector], axis=-1)
...
def quaternion_pose_to_rot_vec_pose(quat_pose):
    ...
    """
    # BS x 4
    vector_and_angle = aikit_aa.quaternion_to_axis_angle(quat_pose[..., 3:])

    # BS x 3
    rot_vec = vector_and_angle[..., :-1] * vector_and_angle[..., -1:]

    # BS x 6
    return aikit.concat([quat_pose[..., 0:3], rot_vec], axis=-1)
