...
def quaternion_pose_to_mat_pose(quat_pose):
    ...
    """
    # BS x 3 x 3
    rot_mat = aikit_rot_mat.quaternion_to_rot_mat(quat_pose[..., 3:])

    # BS x 3 x 1
    rhs = aikit.expand_dims(quat_pose[..., 0:3], axis=-1)

    # BS x 3 x 4
    return aikit.concat([rot_mat, rhs], axis=-1)
...
def euler_pose_to_mat_pose(euler_pose, convention="zyx", batch_shape=None):
    ...
    # BS x 3 x 3
    rot_mat = aikit_rot_mat.euler_to_rot_mat(euler_pose[..., 3:], convention, batch_shape)

    # BS x 3 x 4
    return aikit.concat(
        [rot_mat, aikit.expand_dims(euler_pose[..., 0:3], axis=-1)], axis=-1
    )
...
def rot_vec_pose_to_mat_pose(rot_vec_pose):
    ...
    """
    # BS x 4
    quaternion = aikit_quat.rotation_vector_to_quaternion(rot_vec_pose[..., 3:])

    # BS x 3 x 3
    rot_mat = aikit_rot_mat.quaternion_to_rot_mat(quaternion)

    # BS x 3 x 4
    return aikit.concat(
        [rot_mat, aikit.expand_dims(rot_vec_pose[..., 0:3], axis=-1)], axis=-1
    )
...
def axis_angle_pose_to_mat_pose(axis_angle_pose):
    ...
    """
    # BS x 3 x 3
    rot_mat = aikit_rot_mat.axis_angle_to_rot_mat(axis_angle_pose[..., 3:])

    # BS x 3 x 4
    return aikit.concat(
        [rot_mat, aikit.expand_dims(axis_angle_pose[..., :3], axis=-1)], axis=-1
    )
