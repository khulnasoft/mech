...
from aikit_mech.orientation import axis_angle as aikit_aa
...
def axis_angle_to_quaternion(axis_angle):
    ...
    angle = axis_angle[..., -1:]
    n = aikit.cos(angle / 2)
    e1 = aikit.sin(angle / 2) * axis_angle[..., 0:1]
    e2 = aikit.sin(angle / 2) * axis_angle[..., 1:2]
    e3 = aikit.sin(angle / 2) * axis_angle[..., 2:3]

    # BS x 4
    quaternion = aikit.concat([e1, e2, e3, n], axis=-1)
    return quaternion
...
def polar_axis_angle_to_quaternion(polar_axis_angle):
    ...
    x = aikit.sin(theta) * aikit.cos(phi)
    y = aikit.sin(theta) * aikit.sin(phi)
    z = aikit.cos(theta)

    # BS x 3
    vector = aikit.concat([x, y, z], axis=-1)

    # BS x 4
    return axis_angle_to_quaternion(aikit.concat([vector, angle], axis=-1))
...
def rot_mat_to_quaternion(rot_mat):
    ...
    # BS x 4 x 1
    quat_1 = aikit.concat([qx_1, qy_1, qz_1, qw_1], axis=-2)
    ...
    # BS x 4 x 1
    quat_2 = aikit.concat([qx_2, qy_2, qz_2, qw_2], axis=-2)
    ...
    # BS x 4 x 1
    quat_3 = aikit.concat([qx_3, qy_3, qz_3, qw_3], axis=-2)
    ...
    # BS x 4 x 1
    quat_4 = aikit.concat([qx_4, qy_4, qz_4, qw_4], axis=-2)
    quat_3_or_other = aikit.where(
        rot_mat[..., 1:2, 1:2] > rot_mat[..., 2:3, 2:3], quat_3, quat_4
    )
    quat_2_or_other = aikit.where(
        aikit.logical_and(
    ...
    # BS x 4
    return aikit.where(tr > 0, quat_1, quat_2_or_other)[..., 0]
...
def rotation_vector_to_quaternion(rot_vector):
    ...
    """
    # BS x 1
    theta = (aikit.sum(rot_vector**2, axis=-1, keepdims=True)) ** 0.5

    # BS x 3
    vector = rot_vector / (theta + MIN_DENOMINATOR)

    # BS x 4
    return axis_angle_to_quaternion(aikit.concat([vector, theta], axis=-1))
...
def euler_to_quaternion(euler_angles, convention="zyx", batch_shape=None):
    ...
    return rot_mat_to_quaternion(
        aikit_rot_mat.euler_to_rot_mat(euler_angles, convention, batch_shape)
    )
...
def inverse_quaternion(quaternion):
    ...
    """
    # BS x 1
    sum_of_squares = aikit.sum(quaternion**2, axis=-1)
    vector_conjugate = aikit.concat(
...
def get_random_quaternion(max_rot_ang=math.pi, batch_shape=None):
    ...
    # BS x 3
    quaternion_vector = aikit.random_uniform(low=0, high=1, shape=list(batch_shape) + [3])
    vec_len = aikit.vector_norm(quaternion_vector, axis=-1)
    quaternion_vector /= vec_len + MIN_DENOMINATOR

    # BS x 1
    theta = aikit.random_uniform(
        low=-max_rot_ang, high=max_rot_ang, shape=list(batch_shape) + [1]
    )

    # BS x 4
    return axis_angle_to_quaternion(aikit.concat([quaternion_vector, theta], axis=-1))
...
def scale_quaternion_rotation_angle(quaternion, scale):
    ...
    """
    # BS x 4
    vector_and_angle = aikit_aa.quaternion_to_axis_angle(quaternion)
    ...
    return axis_angle_to_quaternion(
        aikit.concat([vector_and_angle[..., :-1], scaled_angle], axis=-1)
    )
...
def hamilton_product(quaternion1, quaternion2):
    ...
    # BS x 4
    return aikit.concat([term_i, term_j, term_k, term_r], axis=-1)
