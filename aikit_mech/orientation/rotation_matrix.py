...
# local
from aikit_mech.orientation import quaternion as aikit_quat
...
def _x_axis_rotation_matrix(identity_matrix, zeros, sin_theta, cos_theta):
    ...
    rot_x_m = aikit.concat([zeros, cos_theta, -sin_theta], axis=-1)
    rot_x_l = aikit.concat([zeros, sin_theta, cos_theta], axis=-1)
    return aikit.concat([rot_x_u, rot_x_m, rot_x_l], axis=-2)
...
def _y_axis_rotation_matrix(identity_matrix, zeros, sin_theta, cos_theta):
    ...
    """
    rot_y_u = aikit.concat([cos_theta, zeros, sin_theta], axis=-1)
    rot_y_m = identity_matrix[..., 1:2, :]
    rot_y_l = aikit.concat([-sin_theta, zeros, cos_theta], axis=-1)
    return aikit.concat([rot_y_u, rot_y_m, rot_y_l], axis=-2)
...
def _z_axis_rotation_matrix(identity_matrix, zeros, sin_theta, cos_theta):
    ...
    """
    rot_z_u = aikit.concat([cos_theta, -sin_theta, zeros], axis=-1)
    rot_z_m = aikit.concat([sin_theta, cos_theta, zeros], axis=-1)
    rot_z_l = identity_matrix[..., 2:3, :]
    return aikit.concat([rot_z_u, rot_z_m, rot_z_l], axis=-2)
...
def rot_vec_to_rot_mat(rot_vec):
    ...
    """
    # BS x 1
    t = aikit.sum(rot_vec**2, axis=-1, keepdims=True) ** 0.5

    # BS x 3
    u = rot_vec / t

    # BS x 1 x 1
    cost = aikit.expand_dims(aikit.cos(t), axis=-1)
    sint = aikit.expand_dims(aikit.sin(t), axis=-1)
    ux = aikit.expand_dims(u[..., 0:1], axis=-1)
    uy = aikit.expand_dims(u[..., 1:2], axis=-1)
    uz = aikit.expand_dims(u[..., 2:3], axis=-1)
    ...
    # BS x 1 x 3
    top_row = aikit.concat([top_left, top_middle, top_right], axis=-1)
    middle_row = aikit.concat([middle_left, middle_middle, middle_right], axis=-1)
    bottom_row = aikit.concat([bottom_left, bottom_middle, bottom_right], axis=-1)

    # BS x 3 x 3
    return aikit.concat([top_row, middle_row, bottom_row], axis=-2)
...
def quaternion_to_rot_mat(quaternion):
    ...
    """
    # BS x 1 x 1
    a = aikit.expand_dims(quaternion[..., 3:4], axis=-1)
    b = aikit.expand_dims(quaternion[..., 0:1], axis=-1)
    c = aikit.expand_dims(quaternion[..., 1:2], axis=-1)
    d = aikit.expand_dims(quaternion[..., 2:3], axis=-1)
...
def euler_to_rot_mat(euler_angles, convention="zyx", batch_shape=None, device=None):
    ...
    if device is None:
        device = aikit.dev(euler_angles)

    # BS x 1 x 1
    zeros = aikit.zeros(list(batch_shape) + [1, 1], device=device)

    alpha = aikit.expand_dims(euler_angles[..., 0:1], axis=-1)
    beta = aikit.expand_dims(euler_angles[..., 1:2], axis=-1)
    gamma = aikit.expand_dims(euler_angles[..., 2:3], axis=-1)

    cos_alpha = aikit.cos(alpha)
    sin_alpha = aikit.sin(alpha)

    cos_beta = aikit.cos(beta)
    sin_beta = aikit.sin(beta)

    cos_gamma = aikit.cos(gamma)
    sin_gamma = aikit.sin(gamma)

    # BS x 3 x 3
    identity_matrix = aikit.eye(3, 3, batch_shape=batch_shape, device=device)
    ...
    return aikit.matmul(rot_gamma, aikit.matmul(rot_beta, rot_alpha))
...
def target_facing_rotation_matrix(body_pos, target_pos, batch_shape=None, device=None):
    ...
    if device is None:
        device = aikit.dev(body_pos)

    # BS x 3
    up = aikit.tile(
        aikit.reshape(
            aikit.array([0.0, 0.0, 1.0], device=device), [1] * num_batch_dims + [3]
        ),
        batch_shape + [1],
    )

    z = target_pos - body_pos
    z = z / aikit.sum(z**2, axis=-1, keepdims=True) ** 0.5

    x = aikit.cross(up, z)

    y = aikit.cross(z, x)

    x = x / aikit.sum(x**2, axis=-1, keepdims=True) ** 0.5
    y = y / aikit.sum(y**2, axis=-1, keepdims=True) ** 0.5

    # BS x 1 x 3
    x = aikit.expand_dims(x, axis=-2)
    y = aikit.expand_dims(y, axis=-2)
    z = aikit.expand_dims(z, axis=-2)

    # BS x 3 x 3
    return aikit.pinv(aikit.concat([x, y, z], axis=-2))
...
def axis_angle_to_rot_mat(axis_angle):
    ...
    """
    return quaternion_to_rot_mat(aikit_quat.axis_angle_to_quaternion(axis_angle))
...
def get_random_rot_mat(batch_shape=None):
    ...
    return quaternion_to_rot_mat(
        aikit_quat.get_random_quaternion(batch_shape=batch_shape)
    )
