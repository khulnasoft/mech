...
# local
from aikit_mech.orientation import quaternion as aikit_q
...
def rot_mat_to_axis_angle(rot_mat, device=None):
    ...
    """
    quat = aikit_q.rot_mat_to_quaternion(rot_mat)
...
def euler_to_axis_angle(euler_angles, convention="zyx", batch_shape=None, device=None):
    ...
    """
    quat = aikit_q.euler_to_quaternion(euler_angles, convention, batch_shape)
    return quaternion_to_axis_angle(quat, device)
...
def quaternion_to_axis_angle(quaternion, device=None):
    ...
    # BS x 1
    theta = 2 * aikit.acos(aikit.clip(n, 0, 1))
    vector_x = aikit.where(
        theta != 0,
        e1 / (aikit.sin(theta / 2) + MIN_DENOMINATOR),
        aikit.zeros_like(theta, device=device),
    )
    vector_y = aikit.where(
        theta != 0,
        e2 / (aikit.sin(theta / 2) + MIN_DENOMINATOR),
        aikit.zeros_like(theta, device=device),
    )
    vector_z = aikit.where(
        theta != 0,
        e3 / (aikit.sin(theta / 2) + MIN_DENOMINATOR),
        aikit.zeros_like(theta, device=device),
    )

    # BS x 4
    return aikit.concat([vector_x, vector_y, vector_z, theta], axis=-1)
...
def quaternion_to_polar_axis_angle(quaternion, device=None):
    ...
    """
    if device is None:
        device = aikit.dev(quaternion)

    # BS x 4
    vector_and_angle = quaternion_to_axis_angle(quaternion, device)

    # BS x 1
    theta = aikit.acos(vector_and_angle[..., 2:3])
    phi = aikit.atan2(vector_and_angle[..., 1:2], vector_and_angle[..., 0:1])

    # BS x 3
    return aikit.concat([theta, phi, vector_and_angle[..., -1:]], axis=-1)
...
def get_random_axis_angle(batch_shape=None):
    ...
    return quaternion_to_axis_angle(
        aikit_q.get_random_quaternion(batch_shape=batch_shape)
    )
