...
def make_coordinates_homogeneous(coords, batch_shape=None):
    ...
    # BS x 1
    ones = aikit.ones_like(coords[..., 0:1])

    # BS x (D+1)
    return aikit.concat([coords, ones], axis=-1)
...
def make_transformation_homogeneous(matrices, batch_shape=None, device=None):
    ...
    last_row = aikit.tile(
        aikit.reshape(
            aikit.array([0.0, 0.0, 0.0, 1.0], device=device),
    ...
    # BS x 4 x 4
    return aikit.concat([matrices, last_row], axis=-2)
