...
from aikit_demo_utils.aikit_scene.scene_utils import SimObj, BaseSimulator


class DummyCam:
    def __init__(self, interactive):
        self._pos = aikit.array([-1, 0.6, 1.45])
...
class DummyTarget:
    def __init__(self):
        self._pos = aikit.array([0.17179595, -0.01713575, 1.02739596])
    ...
    def __init__(self, interactive, try_use_sim):
        ...
        if self.with_pyrep:
            ...
            self._user_prompt(
                ...
                "aikit_mech.target_facing_rotation_vector "
...
def main(interactive=True, try_use_sim=True, fw=None):
    ...
    for i in range(iterations):
        ...
        tfrm = aikit_mech.target_facing_rotation_matrix(cam_pos, target_pos)
        sim.cam.set_rot_mat(tfrm)
        if not interactive:
            sim.target.set_pos(sim.target.get_pos() + aikit.array([-0.01, 0.01, 0.0]))
        time.sleep(0.05)
    sim.close()
